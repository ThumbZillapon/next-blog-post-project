...
export default function Articles() {
  ...
  useEffect(() => {
    ...
    if (isFirstTimeRender) {
      const fetchCategories = async () => {
        try {
          const categoriesData = await articlesService.getCategories();
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      ...
      try {
        const response = await articlesService.getArticles(page, 6, category);
        if (page === 1) {
          setPosts(response.posts); // Replace posts on the first page load
        } else {
          setPosts((prevPosts) => [...prevPosts, ...response.posts]); // Append on subsequent pages
        }
        setIsLoading(false); // Stop loading
        setHasMore(response.hasMore); // Set hasMore based on response
      } catch (error) {
  ...
  useEffect(() => {
    if (searchKeyword.length > 0) {
      ...
      const fetchSuggestions = async () => {
        try {
          const response = await articlesService.searchArticles(searchKeyword);
          setSuggestions(response.posts); // Set search suggestions
          setIsLoading(false);
        } catch (error) {
